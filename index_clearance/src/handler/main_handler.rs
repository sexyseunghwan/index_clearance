...
impl<I: IndexClearService> MainHandler<I> {
    ...
    pub async fn task_set(&self) -> Result<(), anyhow::Error> {
        ...
        /* 인덱스 삭제 함수. */         
        let send_email_form: Vec<EmailStruct> = self.index_clear_service.delete_index_from_rule().await?;
        
        /* 인덱스 삭제 내역이 있다면, 메일로 post */
        if send_email_form.len() != 0 {
            
            /* smtp client 객체 생성 */
            let smtp_repo = initialize_smtp_clients(
                "./datas/smtp_info.json",
                "./datas/email_receiver_info.json"
            )?;
            
            let cluster_name = self.index_clear_service.get_cluster_name();
            
            /* smtp client 를 통해서 메일 전송 */
            smtp_repo.send_message_to_receivers(&send_email_form, &cluster_name).await?;

        } else {
            info!("Deleted index does not exist.");
        }
