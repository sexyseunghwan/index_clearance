...
pub struct IndexClearServicePub<R: EsRepository> {
    elastic_obj: R,
    clear_index_info: ClusterIndexConfig,
}
...
impl<R: EsRepository> IndexClearServicePub<R> {
    pub fn new(elastic_obj: R) -> Result<Self, anyhow::Error> {
        ...
        // 이쪽에서 에러가 발생하고 있음.
        let cluster_config: ClusterIndexList =
            match read_toml_from_file::<ClusterIndexList>(ELASTIC_INDEX_LIST) {
                Ok(clear_index_list) => clear_index_list,
                Err(e) => {
                    error!("{:?}", e);
                    panic!("{:?}", e)
                }
        ...
        let clear_index_info: ClusterIndexConfig = cluster_config
            .clusters
            .into_iter()
            .find(|cluster| cluster.cluster_name == cluster_name)
            .ok_or_else(|| {
                anyhow!(
                    "No matching cluster found for the given name. : {}",
                    cluster_name
                )
            })?;
...
impl<R: EsRepository + Sync> IndexClearService for IndexClearServicePub<R> {
    ...
    async fn delete_index_from_rule(&self) -> Result<Vec<EmailStruct>, anyhow::Error> {
        ...
        for index_config in target_index {
            ...
            let res = self
                .elastic_obj
                .get_index_belong_pattern(index_pattern)
                .await?;
            ...
            if let Some(index_obj) = res.as_array() {
                for index in index_obj {
                    ...
                    let perserve_days_ago =
                        cur_utc_time - chrono::Duration::days(preserve_term as i64);
                    ...
                    if parsed_date <= perserve_days_ago {
                        ...
                        */
                        if !index_name.contains("log") {
                            info!(
                                "The delete target index name MUST CONTAIN the word 'log'. : {}",
                                index_name
                            );
                            continue;
                        }
            ...
            let cluster_name = self.elastic_obj.get_cluster_name();
            ...
            for delete_index in delete_index_list {
                ...
                match self.elastic_obj.delete_index(&delete_index).await {
                    Ok(_) => {
                        info!("[{}] '{}' index removed", cluster_name, delete_index);

                        let email_struct =
                            match EmailStruct::new(&delete_index, delete_kor_time, true) {
                    ...
                    Err(e) => {
                        error!("{:?}", e);
                        let email_struct =
                            match EmailStruct::new(&delete_index, delete_kor_time, false) {
                                Ok(email_struct) => email_struct,
                                Err(e) => {
                                    error!("{:?}", e);
                                    continue;
